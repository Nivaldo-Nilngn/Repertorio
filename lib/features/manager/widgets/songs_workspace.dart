  ...
  TextSpan buildTextSpan({required BuildContext context, TextStyle? style, required bool withComposing}) {
    ...
    for (final match in pattern.allMatches(textStr)) {
      if (match.start > lastMatchEnd) {
        children.add(TextSpan(text: textStr.substring(lastMatchEnd, match.start), style: style));
      }
      if (match.group(1) != null) {
        ...
        children.add(TextSpan(
          text: match.group(0),
          style: style?.copyWith(color: Colors.tealAccent, fontStyle: FontStyle.italic),
        ));
      }
    ...
    if (lastMatchEnd < textStr.length) {
      children.add(TextSpan(text: textStr.substring(lastMatchEnd), style: style));
    }
...
class _SongsWorkspaceState extends ConsumerState<SongsWorkspace> {
  ...
  Widget _buildEmptyState(ColorScheme colors) {
    ...
    final favoritesCount = ref.watch(songListProvider).value?.where((s) => s.isFavorite).length ?? 0;
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  _buildStatCard('Músicas Salvas', songsCount.toString(), Icons.library_music, colors),
                  _buildStatCard('Favoritas', favoritesCount.toString(), Icons.favorite, colors),
                  _buildStatCard('Repertórios', setlistsCount.toString(), Icons.queue_music, colors),
                ],
              ),
...
''';
  ...
  Widget _buildStatCard(String title, String value, IconData icon, ColorScheme colors) {
    return Container(
      width: 140,
  ...
  Widget build(BuildContext context) {
    ...
    final bool isSelectedSongInList = filteredSongs.any((s) => s.id == selectedSongId);
    
    if (!isSelectedSongInList && !isEditorVisible && filteredSongs.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ref.read(selectedSongIdProvider.notifier).select(filteredSongs.first.id);
        ref.read(editingChordProProvider.notifier).state = filteredSongs.first.content;
      });
    }

    final showPreview = isEditorVisible || (selectedSongId != null && isSelectedSongInList);
    ...
    String titleLabel = activeTab == SidebarTab.favorites ? 'Favoritos' : 'Músicas';
    String? filterLabel;
    if (filter.folderId != null) {
                ...
                Container(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String?>(
                      ...
                      hint: Text('Todos os Repertórios', style: TextStyle(color: colors.onSurfaceVariant, fontSize: 12)),
                      items: [
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          OutlinedButton(
                            onPressed: () {
                              final filterArtist = ref.read(songFilterProvider).artist;
                              final initialArtist = filterArtist ?? 'Artista';
                              ref.read(editingChordProProvider.notifier).state = '''{title: Nova Música}
                          ...
                          child: Row(
                            children: [
                              ...
                              Container(
                                ...
                                child: Text(
                                  song.key,
                                  style: TextStyle(color: colors.onSecondaryContainer, fontSize: 11, fontFamily: 'Consolas'),
                                ),
                              ),
                              const SizedBox(width: 4),
                        ...
                        items: ['Detectar', ..._majorKeys].map((k) {
                          if (k == 'Detectar') {
                            return DropdownMenuItem(
                              value: 'Detectar',
                              child: Text('Detectar', style: TextStyle(color: colors.onSurfaceVariant)),
                            );
                          }
                          final idx = _majorKeys.indexOf(k);
                          return DropdownMenuItem(
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            const SizedBox(width: 8),
                            // CifraClubImporterBtn(), // Descomente para importar novas listas do Cifra Club
                          ],
                        ),
                        if (filterLabel != null)
                          Container(
                      ...
                      Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String?>(
                            ...
                            dropdownColor: colors.surfaceContainer,
                            style: TextStyle(color: colors.onSurface, fontSize: 12),
                            hint: Text('Filtrar por Repertório', style: TextStyle(color: colors.onSurfaceVariant, fontSize: 12)),
                            items: [
                              const DropdownMenuItem<String?>(
                                value: null,
                                child: Text('Todos os Repertórios'),
                      ...
                      return Center(
                        child: Padding(
                          ...
                          child: Column(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Text('Nenhuma música encontrada.', style: TextStyle(color: colors.onSurfaceVariant), textAlign: TextAlign.center),
                              const SizedBox(height: 16),
                              OutlinedButton(
                                onPressed: () {
                                  final filterArtist = ref.read(songFilterProvider).artist;
                              ...
                              child: Row(
                                children: [
                                  Container(
                                    ...
                                    height: 36,
                                    decoration: BoxDecoration(
                                      color: isActive ? colors.primary.withOpacity(0.2) : colors.surfaceContainerHighest,
                                      borderRadius: BorderRadius.circular(8),
                                    ),
                                    child: Icon(song.isFavorite ? Icons.favorite : Icons.music_note, color: isActive ? colors.primary : colors.onSurfaceVariant, size: 18),
                                  ),
